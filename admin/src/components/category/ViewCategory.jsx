...
const ViewCategory = ({token, categoryId, isActive, includeDeleted }) => {
  ...
  if (!categoryId) return <div className="p-4">Enter category ID to search.</div>;
  ...
  return (
    <div className="p-4 bg-white shadow-md rounded-lg">
      ...
      <div className="mb-4">
        ...
        <p><strong>Modified At:</strong> {new Date(category.modifiedAt).toLocaleString()}</p>
      </div>

      {mainImage && (
        <div className="mb-4">
          <h3 className="text-xl font-semibold mb-2">Main Image</h3>
          <img src={mainImage.url} alt={category.name} className="w-48 h-48 object-cover rounded-md shadow-sm" />
        </div>
      )}
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {category.images.map((img, index) => (
              <img key={index} src={img.url} alt={`Category ${category.name} image ${index + 1}`} className="w-24 h-24 object-cover rounded-md shadow-sm" />
            ))}
          </div>
        </div>
      )}
          ...
          <ul className="list-disc pl-5">
            {category.subCategories.map((sub) => (
              <li key={sub.id} className="mb-2">
                ...
                <p><strong>Active:</strong> {sub.isActive ? "Yes" : "No"}</p>
                {sub.images && sub.images.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {sub.images.map((subImg, idx) => (
                      <img key={idx} src={subImg.url} alt={`Subcategory ${sub.name} image ${idx + 1}`} className="w-16 h-16 object-cover rounded-md" />
                    ))}
                  </div>
                )}
              </li>
