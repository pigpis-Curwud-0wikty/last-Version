  ...
  useEffect(() => {
    if (products.length > 0) {
      ...
      if (found) {
        ...
        // Fetch discount information when product is found
        fetchDiscountInfo(found._id);
      } else {
        setProductData(null);
      }
    }
  ...
  const fetchDiscountInfo = async (id) => {
    ...
    try {
      const response = await axios.get(
        `${backendUrl}/api/Products/${id}/Discount`
      );
      if (response.data?.responseBody?.data) {
        setDiscountInfo(response.data.responseBody.data);
        console.log("Discount info:", response.data.responseBody.data);
      }
    } catch (error) {
